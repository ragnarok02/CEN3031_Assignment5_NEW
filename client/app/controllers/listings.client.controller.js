  ...
  function($scope, $location, $stateParams, $state, Listings){
    $scope.find = function() {
      ...
      Listings.getAll().then(function(response) {
        ...
        for(var i = 0; i < $scope.listings.length; i++) {
        	if($scope.listings[i].address != null) {
        		$scope.markers.push({
        			id: i,
    ...
    $scope.update = function(isValid) {
      /*
        ...
        successfully finished, navigate back to the 'listings.list' state using $state.go(). If an error 
    ...
    $scope.remove = function() {
      /*
        Implement the remove function. If the removal is successful, navigate back to 'listings.list'. Otherwise, 
        display the error. 
       */
    	var id = $stateParams.listingId;
